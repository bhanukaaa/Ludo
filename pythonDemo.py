...
# seed = 3392489354586
# seed = 6515047812987 # BHAWANA BLOCK SIZE LOGIC CONCERN
...
class Piece:
    def __init__(self, cellNum, tokenName, clockwise, captured, approachPass, bhawanaMultiplier, kotuwaBriefing):
...
class stdCell:
    def __init__(self, playerID, count, clockwise):
...
def blockDirection(cell, pID):
    # + Returns +
    # False if greatest distance to home in a block is in anticlockwise
    # True if greatest distance to home in a block is in clockwise
    ...
    for token in tokens[pID]:
        if token.cellNum == cell:
            if token.clockwise:
                ...
                # anticw piece that hasn't passed approach is always highest distance
                if not token.approachPass: return False
                anticw = True
...
def capture(cell, capturerID, capturingName, capturedID, block):
    ...
    for capturedToken in tokens[capturedID]:
        if capturedToken.cellNum == cell:
            ...
            capturedToken.clockwise = True
            capturedToken.captured = False
            capturedToken.approachPass = False
...
def moveToken(src, dest, pID, tID, direct):
    ...
    if stdPath[dest].playerID == -1:
        ...
        print(f"\tUpdating Block {blockName(pID, dest)} Direction: {stdPath[dest].clockwise}")
        return 1
...
def moveBlock(src, dest, pID, direct):
    ...
    if stdPath[dest].playerID == -1:
        ...
        # occupied by opposing color
        if stdPath[dest].count == size or stdPath[dest].count == 1: # block can capture single piece
            # can capture
            print(f"{pName} moves block {bName} from location {src} to {dest} by {dist} units in {directionStr} direction")
    ...
    for token in tokens[pID]:
        if token.cellNum == cell:
            if bhawanaEffect == 1:
                ...
                token.bhawanaMultiplier = -4
    teleport(cell, 9, pID, mName, "Mystery Cell" , 'Bhawana')


def kotuwa(pID, cell, mName, fromPitaKotuwa):
    for token in tokens[pID]:
        if token.cellNum == cell:
    ...
    if fromPitaKotuwa:
        teleport(cell, 27, pID, mName, "Pita Kotuwa" , 'Kotuwa')
    else:
        teleport(cell, 27, pID, mName, "Mystery Cell" , 'Kotuwa')
...
def kotuwaBreak(pID):
    ...
    for token in tokens[pID]:
        if token.cellNum == 27 and token.kotuwaBriefing > 0:
            ...
            token.cellNum = -1
            token.clockwise = True
            token.captured = False
            token.approachPass = False
    ...
    if stdPath[27].count == 0:
        stdPath[27] = stdCell(-1, 0, 0)
    playerSummary(pID)


def pitaKotuwa(pID, cell, mName):
    for token in tokens[pID]:
        if token.cellNum == cell:
            if token.clockwise:
                ...
                kotuwa(pID, cell, mName, True)
                return # potential logic error, as if even one piece is moving anticlockwise, the whole block will move to kotuwa

    teleport(cell, 46, pID, mName, "Mystery Cell" , 'Pita-Kotuwa')
...
def landMystery(pID, mID, cell):
    ...
    size = stdPath[cell].count
    mystery = rollDice()
    match mystery: # use switch-case in C
        case 1:
            # teleport to Bhawana
            print(f"and is teleported to Bhawana")
            if stdPath[9].count <= 1 or (size != 1 and stdPath[9].count == size):
                bhawana(pID, cell, mName)

        case 2:
            # teleport to Kotuwa
            print(f"and is teleported to Kotuwa")
            if stdPath[27].count <= 1 or (size != 1 and stdPath[27].count == size):
                kotuwa(pID, cell, mName, False)

        case 3:
            # teleport to Pita-Kotuwa
            print(f"and is teleported to Pita-Kotuwa")
            if stdPath[46].count <= 1 or (size != 1 and stdPath[46].count == size):
                pitaKotuwa(pID, cell, mName)

        case 4:
            # teleport to Base
            print(f"and is teleported to {players[pID].name} Base")
            stdPath[cell] = stdCell(-1, 0, 0)
            for token in tokens[pID]:
                if token.cellNum == cell:
                    token.cellNum = -1
                    token.clockwise = True
        ...
        case 5:
            ...
            teleport(cell, players[pID].startingCell, pID, mName, "Mystery Cell" , "Starting Cell")

        case 6:
            # teleport to Approach
            print(f"and is teleported to {players[pID].name} Approach Cell ({players[pID].approachCell})")
            teleport(cell, players[pID].approachCell, pID, mName, "Mystery Cell" , "Approach Cell")
...
def tokenTurn(pID, tID, diceValue, mysteryCell):
    ...
    if currPos == -1 and diceValue == 6:
        ...
        stdPath[pStart].clockwise = blockDirection(pStart, pID)
        ################################################
            ...
            if (tDir == 1 or tAppP > 0):
                ...
                if tCapt > 0:
                    ...
                    if stdPath[currPos].count == 0:
                        stdPath[currPos] = stdCell(-1, 0, 0)
                    else:
                        stdPath[currPos].clockwise = blockDirection(currPos, pID)
                        print(f"\tUpdating Block {blockName(pID, currPos)} Direction: {stdPath[currPos].clockwise}")
...
def blockTurn(pID, bID, diceValue, mysteryCell):
    ...
    bDir = stdPath[currPos].clockwise
    ...
    if currPos < 52:
        ...
        if distToApproach < distToNextPos:
            # passing approach
            # block can only enter hmstrt if ALL members are eligible to enter

            eligible = True
            for token in tokens[pID]:
                if token.cellNum != currPos: continue
                # Note: Checking Piecewise Eligibility, not with Block Direction
                if not token.captured:
                    eligible = False
                    break
                
                if not token.clockwise and not token.approachPass:
                    eligible = False
                    break
            ...
            if eligible:
                ...
                # print block is moving to home straight

                for token in tokens[pID]:
                    if token.cellNum == currPos:
                        token.cellNum = nextPos
                
                stdPath[currPos] = stdCell(-1, 0, 0)
                return 1
            else:
                # print block is not eligible so moving forward
                pass
...
def greenBehaviour(diceV):
    ...
    for i in range(4):
        ...
        if stdPath[token.cellNum].count > 1:
            ...
            players[0].choiceWeights[i] = 60 - distanceTo(token.cellNum, pApproach, token.clockwise)

        nextCell = cellAtMove(token.cellNum, diceV, token.clockwise)
...
def yellowBehaviour(diceV):
    ...
    for i in range(4):
        ...
        if token.clockwise == -1 and token.approachPass < 1:
...
def redBehaviour(diceV):
    ...
    for i in range(4):
                ...
                if startingCellStatus == -1:
                    ...
                    for oppToken in tokens[targetPlayer]:
                        if oppToken.cellNum != pStart:
                            ...
                            players[3].choiceWeights[i] = 55 - distanceTo(pStart, players[targetPlayer].approachCell, oppToken.clockwise)
                            # opponent is further from home if anticlockwise and not passed approach
                            if not oppToken.clockwise and not oppToken.approachPass:
        ...
        for oppToken in tokens[targetPlayer]:
            ...
            # opponent closer to home, higher value
            players[3].choiceWeights[i] = 55 - distanceTo(nextCell, players[targetPlayer].approachCell, oppToken.clockwise)
...
tokens = [
    [
        Piece(-1, GREEN + BOLD + "G1" + RESET, True, False, False, 0, 0),
        Piece(-1, GREEN + BOLD + "G2" + RESET, True, False, False, 0, 0),
        Piece(-1, GREEN + BOLD + "G3" + RESET, True, False, False, 0, 0),
        Piece(-1, GREEN + BOLD + "G4" + RESET, True, False, False, 0, 0)
    ],
    [
        Piece(-1, YELLOW + BOLD + "Y1" + RESET, True, False, False, 0, 0),
        Piece(-1, YELLOW + BOLD + "Y2" + RESET, True, False, False, 0, 0),
        Piece(-1, YELLOW + BOLD + "Y3" + RESET, True, False, False, 0, 0),
        Piece(-1, YELLOW + BOLD + "Y4" + RESET, True, False, False, 0, 0)
    ],
    [
        Piece(-1, BLUE + BOLD + "B1" + RESET, True, False, False, 0, 0),
        Piece(-1, BLUE + BOLD + "B2" + RESET, True, False, False, 0, 0),
        Piece(-1, BLUE + BOLD + "B3" + RESET, True, False, False, 0, 0),
        Piece(-1, BLUE + BOLD + "B4" + RESET, True, False, False, 0, 0)
    ],
    [
        Piece(-1, RED + BOLD + "R1" + RESET, True, False, False, 0, 0),
        Piece(-1, RED + BOLD + "R2" + RESET, True, False, False, 0, 0),
        Piece(-1, RED + BOLD + "R3" + RESET, True, False, False, 0, 0),
        Piece(-1, RED + BOLD + "R4" + RESET, True, False, False, 0, 0)
    ]
]

# array to represent stdPath
# player ID, number of tokens in cell, direction of token / block
stdPath = [stdCell(-1, 0, 0)] * 52
...
def debugging():
    for i in range(52):
        ...
        if not (0 <= stdPath[i].playerID <= 3):
            ...
            stdPathView()
            time.sleep(25)
        if not (-1 <= stdPath[i].clockwise <= 1):
...
def endOfRound(roundNum, mysteryRound, currMystery):
    ...
    for p in range(4):
        ...
        print("==========================")
        for t in tokens[p]:
            print(f"Piece {t.tokenName} [capt: {t.captured}, appr: {t.approachPass}, dir: {t.clockwise}] -> ", end="")
            if t.cellNum == -1:
...
def spawnMysteryCell(currMystery):
    while True:
        ...
        if new != currMystery and stdPath[new].count == 0:
