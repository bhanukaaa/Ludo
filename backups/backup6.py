...
def playTurn(pID, roundNum, sixRepeats, mysteryCell):
    ...
    for index, token in enumerate(tokens[pID]):
        if token.cellNum == -1:
            ...
            # else: # TEMPORARY: FORCE PLAYERS TO FAVOR BLOCKS
            #     options.append(index)
    ...
    if playerChoice < 4:
        # picked a token
        choiceRes = tokenTurn(pID, playerChoice, diceValue, mysteryCell)
    else:
        # picked a block
        choiceRes = blockTurn(pID, playerChoice, diceValue, mysteryCell)
...
def greenBehaviour(options):
    return 0

def yellowBehaviour(options):
    return 0

def blueBehaviour(options):
    return 0

def redBehaviour(options):
    # capturing >>>
...
def playerSimulation(pID, options):
    match pID:
        case 0:
            # green player
            return greenBehaviour(options)
        case 1:
            # yellow player
            return yellowBehaviour(options)
        case 2:
            # blue player
            return blueBehaviour(options)
        case 3:
            # red player
            return redBehaviour(options)
