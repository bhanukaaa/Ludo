    ...
    if stdPath[dest][0] == -1:
        ...
        if stdPath[dest][1] == 1:
            ...
            if src != -1:
                ...
                print(f"{pName} piece {tName} is blocked from moving from the Base to {dest}, by {playerInfo[blockingID][0]} block {blockName(blockingID, dest)}")
            return 3
...
def tokenTurn(pID, tID, diceValue):
    ...
    if 0 <= currPos < 52:
        ...
        if allowedPos == currPos:
            ...
            print(f"{pName} piece {tName} is blocked from moving from {currPos} to {nextPos} by {playerInfo[blocker][0]} ", end="")
    ...
    if currPos < 52:
        ...
        if allowedPos == -1:
            ...
            if stdPath[nextPos][1] != bCount:
                ...
                print(f"{pName} block {bName} is blocked from moving from {currPos} to {nextPos} by {playerInfo[blocker][0]}", end="")
            ...
            while True:
                ...
                if turnRes == 2:
                    ...
                    if playerInfo[currPlayer][5] == 4:
                        ...
                        print(f"{playerInfo[currPlayer][0]} Player Wins!!! Round: {roundNum}")
                        print("#####################################")
                ...
                if turnRes == 6:
                    print(f"\n{playerInfo[currPlayer][0]} Player is awarded a bonus roll for rolling 6")
                    sixRepeats += 1
                else:
                    print(f"\n{playerInfo[currPlayer][0]} Player is awarded a bonus roll for capturing an opponent")
                    sixRepeats = 0
...
def findStarting():
    ...
    while first == -1:
        for i in range(4):
            roll = rollDice()
            print(f"{playerInfo[i][0]} rolls {roll}")
            time.sleep(0.15)
            rolls[i] += roll
            if rolls[i] > maxVal:
                maxVal = rolls[i]
    ...
    time.sleep(0.5)
    print(f"\n{playerInfo[first][0]} has the highest roll and will begin the game")
    print(f"The order of a single round will be {playerInfo[first][0]}, {playerInfo[(first+1) % 4][0]}, {playerInfo[(first+2) % 4][0]}, {playerInfo[(first+3) % 4][0]}")
    time.sleep(0.5)
    return first
