...
HomeStrtCell homeStraights[4][5]; // data about homeStraights for quick reference
...
void blockName(short pID, short blockCell) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (tokens[pID][i].cellNum == blockCell) {
...
void playerSummary(short pID) {
    ...
    Player plyr = players[pID];
    printf("%s Player now has %d/4 Pieces on the Board, and %d/4 Pieces in the Base", plyr.name, plyr.onBoard, plyr.inBase);
    if (plyr.inHome) {
        // any pieces in home
        printf(", and %d/4 Pieces in Home", plyr.inHome);
    }
    printf("\n");
}
...
void capture(short cell, short capturerID, short tokenIDblockCell, short capturedID, bool block) {
    ...
    for (int i = 0; i < 4; i++) {
        if (tokens[capturedID][i].cellNum == cell) {
            // in capturing cell
...
short moveBlock(short src, short dest, short pID, bool clockwise) {
    ...
    if (stdPath[dest].playerID == -1) {
        ...
        for (int i = 0; i < 4; i++) {
            if (tokens[pID][i].cellNum == src) tokens[pID][i].cellNum = dest;
        }
        return 0;
    }
...
void kotuwaBreak(short pID) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (token->cellNum == 27 && token->kotuwaBrief > 0) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (token->cellNum == mCell) {
            ...
            if (token->clockwise) {
                ...
                return; // potential logic error, as if even one piece is moving anticw, the whole block will tp to kotuwa
            }
        }
    }
...
void landMystery(short pID, short mID, short mCell) {
    ...
    printf("\a%s player lands on a mystery cell and is teleported to ", players[pID].name);
    switch (mystery) {
        case 1:
            // teleport to Bhawana
            printf("Bhawana\n");
...
short tokenTurn(short pID, short tID, short diceValue, short mysteryCell) {
    ...
    //       blue: 85-89, 90
    //       red: 95-99, 100
    short pHome = 70 + 10 * pID;
    short pHomeStrt = 65 + 10 * pID;
...
short blockTurn(short pID, short bID, short diceValue, short mysteryCell) {
    ...
    short bSize = stdPath[currPos].count;
    bool bClockwise = stdPath[currPos].clockwise;

    if (diceValue < bSize) {
        printf("%s tries to move block ", pName);
        blockName(pID, currPos);
        printf("\tDice roll of %hd is not sufficient to move block of size %hd\n", diceValue, bSize);
        return -1;
    }
...
void breakPlayerBlocks(short pID) {
    ...
    // subarray - location, count, clockwise, anticlockwise
    short blocks[][] = {{-1,0,0,0}, {-1,0,0,0}};
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (blocks[0][0] == -1 || blocks[0][0] == token.cellNum) {
            blocks[0][0] = token.cellNum;
            blocks[0][1] += 1;
            if (token.clockwise) blocks[0][2] += 1;
            else blocks[0][3] += 1;
        } else {
            blocks[1][0] = token.cellNum;
            blocks[1][1] += 1;
            if (token.clockwise) blocks[1][2] += 1;
            else blocks[1][3] += 1;
        }
    }

    // 4 possibilities;
    //   1 block of 2, 1 block of 3, 1 block of 4, 2 blocks of 2
    if (blocks[1][0] == -1) {
        // one block
    } else {
        // two blocks of 2
...
void updateChoiceWeights(short pID, short diceV, short mysteryCell); // temp prototype
...
short playTurn(short pID, short roundNum, short sixRepeats, short mysteryCell) {
    ...
    if (diceValue == 6 && sixRepeats == 2) {
        // breakPlayersBlocks(pID);
        printf("%s Player rolled three 6's in a row, Ignoring throw and moving on to the next player\n", pName);
        return 0;
    }
...
void greenBehaviour(short diceV) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (token.cellNum == 70) continue; // in home
        ...
        if (52 < token.cellNum && token.cellNum < 70) {
            // home straight
...
void yellowBehaviour(short diceV) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (token.cellNum == 80) continue; // in home
        ...
        if (52 < token.cellNum && token.cellNum < 80) {
...
void blueBehaviour(short diceV, short mysteryCell) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (token.cellNum == 90) continue; // in home
        ...
        if (52 < token.cellNum && token.cellNum < 90) {
...
void redBehaviour(short diceV) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (token.cellNum == 100) continue; // in home
        ...
        if (52 < token.cellNum && token.cellNum < 100) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            // NOTE: MIGHT NEED UPDATE IF ANTICLOCKWISE PIECES ARE CONSIDERED FURTHER AWAY
        }
    }
}
...
void endOfRound(short roundNum, short mysteryRound, short currMystery) {
    ...
    for (int p = 0; p < 4; p++) {
        ...
        for (int t = 0; t < 4; t++) {
            ...
            else if (tokens[p][t].cellNum == 70 + 10 * p) printf("Home\n");
            else if (tokens[p][t].cellNum < 52) printf("Cell %hd\n", tokens[p][t].cellNum);
            else printf("%s|HomePath|%hd\n", players[p].name, tokens[p][t].cellNum - (65 + 10 * p));
...
bool checkEndCondition() {
    ...
    for (int p = 0; p < 4; p++) {
        for (int t = 0; t < 4; t++) {
            if (tokens[p][t].cellNum == -1) return false;
        }
    }
    return true;
}
...
void gameLoop(short starter) {
    ...
    while (1) {
        ...
        // srand seed for development purposes
        if (DEV_MODE) printf("\t>>>>> Current SRand Seed: %d\n", seed);
            ...
            while (1) {
                ...
                if (turnRes == 2 || turnRes == 7) {
                    ...
                    if (players[currPlayer].inHome == 4) {
                        if (DEV_MODE) endOfRound(roundNum, mysteryRound, currMystery);
...
void gameStart() {
    ...
    seed = time(NULL);
    // seed = 1724165246; // BLOCK MEETUP EDGE CASE
    // seed = 1724151279; // SOME WEIRD BLUE WINNIN CASE
    // seed = 1724165680;
