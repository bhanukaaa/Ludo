# ToDo;
# convert stdPath array to use classes
# Mystery Cells
# Bhavana
# Kotuwa
# Pita-Kotuwa
...
def playTurn(pID, roundNum, sixRepeats):
    ...
    if not options:
        # no options possible
        print(f"{pName} {GRAY}has no possible moves, Ignoring the throw and moving on to the next player{RESET}")
        return 0
...
tokens = [
    [
        Piece(-1, GREEN + BOLD + "G1" + RESET, 1, 0, 0, 0),
        Piece(-1, GREEN + BOLD + "G2" + RESET, 1, 0, 0, 0),
        Piece(-1, GREEN + BOLD + "G3" + RESET, 1, 0, 0, 0),
        Piece(-1, GREEN + BOLD + "G4" + RESET, 1, 0, 0, 0)
    ],
    [
        Piece(-1, YELLOW + BOLD + "Y1" + RESET, 1, 0, 0, 0),
        Piece(-1, YELLOW + BOLD + "Y2" + RESET, 1, 0, 0, 0),
        Piece(-1, YELLOW + BOLD + "Y3" + RESET, 1, 0, 0, 0),
        Piece(-1, YELLOW + BOLD + "Y4" + RESET, 1, 0, 0, 0)
    ],
    [
        Piece(-1, BLUE + BOLD + "B1" + RESET, 1, 0, 0, 0),
        Piece(-1, BLUE + BOLD + "B2" + RESET, 1, 0, 0, 0),
        Piece(-1, BLUE + BOLD + "B3" + RESET, 1, 0, 0, 0),
        Piece(-1, BLUE + BOLD + "B4" + RESET, 1, 0, 0, 0)
    ],
    [
        Piece(-1, RED + BOLD + "R1" + RESET, 1, 0, 0, 0),
        Piece(-1, RED + BOLD + "R2" + RESET, 1, 0, 0, 0),
        Piece(-1, RED + BOLD + "R3" + RESET, 1, 0, 0, 0),
        Piece(-1, RED + BOLD + "R4" + RESET, 1, 0, 0, 0)
    ]
]

# array to represent stdPath
# player ID, number of tokens in cell, direction of token / block
stdPath = [[-1, -1, 0]] * 52 # change to use a class
...
def stdPathView():
    ...
    for i in range(52):
        if stdPath[i][2] == 1:
            ...
            ddd = ' '
        print(f"{str(i).zfill(2)} P:{BOLD}{stdPath[i][0] if stdPath[i][0] != -1 else ' '}{RESET} | C:{BOLD}{stdPath[i][1] if stdPath[i][1] != -1 else ' '}{RESET} | D:{BOLD}{ddd}{RESET} \t", end="")
    ...
    while True:
        ...
        for _ in range(4):
            ...
            while True:
                ...
                if turnRes == 6:
                    print(f"\n{ITALICS}{players[currPlayer].name} {GRAY}{ITALICS}Player is awarded a bonus roll for rolling 6 {RESET}")
                    sixRepeats += 1
                else:
                    print(f"\n{ITALICS}{players[currPlayer].name} {GRAY}{ITALICS}Player is awarded a bonus roll for capturing an opponent{RESET}")
                    sixRepeats = 0
