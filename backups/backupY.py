...
class Player:
    def __init__(self, name, starting, approach, inBase, onBoard, inHome, restricted, prevThree):
        self.name = name
...
def landMystery(pID, mID, cell):
    if mID < 4:
        ...
        mName = "Block " + blockName(pID, tokens[pID][mID - 5].cellNum)
...
def playTurn(pID, roundNum, sixRepeats, mysteryCell):
    ...
    playerChoice = playerSimulation(pID, diceValue)
...
def yellowBehaviour(diceV):
    ...
    for i in range(4):
        ...
        elif 52 < token.cellNum < 80:
            # home straight
            if token.cellNum + diceV <= 80:
                moveValue[i] = token.cellNum
            continue
        ...
        #     moveValue[i] = 55 - distanceTo(nextCell, players[targetPlayer].approachCell, oppToken.direction)


def blueBehaviour(diceV):
    return 0
...
def redBehaviour(diceV):
                ...
                if startingCellStatus == -1:
                    ...
                    for oppToken in tokens[targetPlayer]:
                        if oppToken.cellNum != pStart:
                            # opponent closer to home, higher value
                            moveValue[i] = 55 - distanceTo(pStart, players[targetPlayer].approachCell, oppToken.direction)
            continue
    ...
    # value for each token will be
    # -1 - not possible move
    # 0 - move would result in block forming
    # 1 - onboard but move wouldn't capture opponent
    # 2 - moving out of base
    # >3 - capturing opponent, greater value the closer opponent is to home
    highVal = -1
    highIndex = -1
    for i in range(4):
        if moveValue[i] > highVal:
            highVal = moveValue[i]
            highIndex = i

    return highIndex

def playerSimulation(pID, diceV):
    if diceV != 6 and players[pID].onBoard == 0:
        # no options on board
        return -1
    ...
    match pID:
        # case 0:
        #     # green player
        #     return greenBehaviour(diceV)
        # case 1:
        #     # yellow player
        #     return yellowBehaviour(diceV)
        # case 2:
        #     # blue player
        #     return blueBehaviour(diceV)
        case 3:
            # red player
            return redBehaviour(diceV)
        case _:
            options = list()
            for i in range(4):
                token = tokens[pID][i]
                if token.cellNum == -1 and diceV == 6: options.append(i)
                elif 0 <= token.cellNum < 52:
                    options.append(i)
                    if stdPath[token.cellNum].count > 1: options.append(i + 4)
                elif token.cellNum != 70 + 10 * pID: options.append(i)
...
players = [
    Player(GREEN + "Green" + RESET, 41, 39, 4, 0, 0, 0, False),
    Player(YELLOW + "Yellow" + RESET, 2, 0, 4, 0, 0, 0, False),
    Player(BLUE + "Blue" + RESET, 15, 13, 4, 0, 0, 0, False),
    Player(RED + "Red" + RESET, 28, 26, 4, 0, 0, 0, False),
]
